...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id']) && !isset($_POST['search'])) {
    ...
    $stmt = $conn->prepare(
        "UPDATE employees
         SET department_id=?, ename=?, dob=?, gender=?, email=?, pnumber=?, address=?, designation=?, salary=?, joining_date=?, aadhar=?
         WHERE id=?"
    );
    $stmt->bind_param(
        "sssssssssdss",
...
if ($search !== '') {
    ...
    $stmt = $conn->prepare(
        "SELECT * FROM employees
         WHERE ename LIKE ? OR email LIKE ? OR designation LIKE ? OR department_id LIKE ?"
    );
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        th { background: #111827; color: #f9fafb; font-weight: 600; }
        td { background: #f9fafb; }
        td a { color: #111827; text-decoration: none; }
        td a:hover { color: #2563eb; }
        i.fas.fa-trash { color: #b91c1c; cursor: pointer; }
        i.fas.fa-trash:hover { color: #ef4444; }
        i.fas.fa-edit { color: #065f46; }
        i.fas.fa-edit:hover { color: #10b981; }
        i.fas.fa-plus { color: #1d4ed8; }
        i.fas.fa-plus:hover { color: #3b82f6; }
        ...
        .delete-close:hover { background: #ef4444; color: #fff; }
        .delete-modal-icon {
        ...
        .delete-modal-icon i { font-size: 22px; color: #dc2626; }
        .delete-title { font-size: 18px; font-weight: 700; text-align: center; margin-bottom: 8px; }
        .delete-text { font-size: 14px; text-align: center; color: #6b7280; margin-bottom: 20px; }
        .delete-actions { display: flex; gap: 10px; }
        .btn-delete-cancel, .btn-delete-confirm {
            flex: 1; padding: 10px; border-radius: 6px; font-weight: 600; cursor: pointer; transition: all 0.2s ease;
        }
        .btn-delete-cancel { background: #fff; border: 1px solid #d1d5db; }
        .btn-delete-cancel:hover { background: #f3f4f6; }
        .btn-delete-confirm { background: #dc2626; border: 1px solid #dc2626; color: #fff; }
        .btn-delete-confirm:hover { background: #b91c1c; }

        @keyframes modalSlideIn { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } }
        @media (min-width:768px) { .table-container { padding: 30px 24px 40px; } table { min-width: 0; } }
        @media (max-width:480px) { th, td { padding: 8px 6px; font-size: 0.8rem; } .delete-modal { min-width: 280px; margin:0 16px; } }
    </style>
</head>
<body>
<?php
...
<div class="table-container">
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; flex-wrap:wrap; gap:12px;">
        <h1 style="margin:0;">Employees Data</h1>
        <form method="get">
            <input type="text" name="search" placeholder="Dept ID/Name" value="<?php echo htmlspecialchars($search, ENT_QUOTES,'UTF-8'); ?>" style="padding:6px 8px; border-radius:4px; border:1px solid #ccc;">
    ...
    <table>
        <tr>
            <th>Full Name</th><th>Department ID</th><th>DOB</th><th>Gender</th><th>Email</th><th>Phone</th><th>Address</th><th>Designation</th><th>Salary</th><th>Joining Date</th><th>Aadhar/ID</th><th>Update</th><th>Delete</th><th>Add New</th>
        </tr>
        <?php
        if ($result && $result->num_rows > 0) {
            ...
            echo "<tr><td colspan='14'>No data found</td><td><a href='form.php'><i class='fas fa-plus'></i></a></td></tr>";
        }
        $conn->close();
        ?>
    </table>

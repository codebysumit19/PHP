...
.search-box:focus { outline:none; border-color:#68A691; }
.clear-search {
    position:absolute;
    right:10px;
    top:50%;
...
td { background:#f9fafb; transition:background 0.2s; }
tbody tr:hover td { background:#e0f2f1; }
...
.btn-delete-cancel { background:#fff; border:1px solid #d1d5db; color:#374151; }
...
/* Hide the browser's default clear (X) button in search input */
...
document.getElementById('delete-confirm').onclick=function(){ if(deleteId) window.location.href="?id="+encodeURIComponent(deleteId); };
document.getElementById('delete-overlay').addEventListener('click',function(e){ if(e.target===this) hideDeleteModal(); });
...
function resetTable() {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        for (let c = 0; c < cells.length - 3; c++) { // Only reset data cells
